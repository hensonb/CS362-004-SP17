...
int initializeGame(int numPlayers, int kingdomCards[10], int randomSeed,
  ...
  // NOTE: THIS IS INCORRECT BEHAVIOR that doesn't match the rules of the game!!
...
int handCard(int handPos, struct gameState *state) {
  ...
  return state->hand[whoseTurn(state)][handPos];
}
...
int fullDeckCount(int player, int card, struct gameState *state) {
	// how many CARD cards are in the deck?
  int i;
  int count = 0;
...
int endTurn(struct gameState *state) {
  ...
  // why the fuck wouldn't you just use a MODULUS operator!?
  ...
  //Update money
  // NOTE: this SHOULD happen here, it already happens after any card is played, but they may not play a card
  // also multiple calls to this will have no adverse effects
  updateCoins(state->whoseTurn, state , 0);
...
int isGameOver(struct gameState *state) {
  ...
  j = 0;
  // this should iter until i <= treasure_map... NOT 25!!
  for (i = 0; i < 25; i++)
    {
...
int scoreFor (int player, struct gameState *state) {
  ...
  //score from deck
  // BUG: iterating to discardCount, not deckCount
  for (i = 0; i < state->discardCount[player]; i++)
    {
...
{        int count;
  ...
  if (state->deckCount[player] <= 0){//Deck is empty
    ...
    state->discardCount[player] = 0; // redundant
    ...
    if (DEBUG){//Debug statements
      ...
      printf("Card being drawn: %d\n", state->deck[player][deckCounter - 1]);
	  	// for (i = 0; i < state->deckCount[player]; i++) {
			// printf("Deck%i = %i, ",i,state->deck[player][i]);
		// }
		// printf("\n");
    }
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    {
    case adventurer:
	  // dig for top 2 treasure cards
	  // moved to distinct function
	  ret = playAdventurer(state, handPos, currentPlayer);
	  return ret;
      ...
      for (i = 0; i < state->handCount[currentPlayer]; i++)
        {
          if (state->hand[currentPlayer][i] == j)
            {
              discardPlayedCard(i, currentPlayer, state, 0);  // BUG: normal discard instead of trash
              break;
            }
        }
    ...
    case remodel:
	  ...
	  // moved to a discrete function
      ret = playRemodel(state, handPos, currentPlayer, choice1, choice2);
      return ret;
    ...
    case smithy:
      //+3 Cards
	  // moved to distinct function 
      ret = playSmithy(state, handPos, currentPlayer);
      return ret;
                
    case village:
	  // +1 card, +2 actions
	  // moved to distinct function
      ret = playVillage(state, handPos, currentPlayer);
	  return ret;
	  
    case baron:
		// +1 buy
		// may discard an estate for +4 coins... if not, gain an estate for free
		// moved to distinct function
		ret = playBaron(state, handPos, currentPlayer, choice1);
		return ret;
                  ...
                  if ( state->handCount[i] > 4 )
                    {
                      ...
                      while( state->handCount[i] > 0 )
                        {
                          discardPlayedCard(handPos, i, state, 0);
                        }
    ...
    case steward:
      if (choice1 == 1)
      ...
      else
        {
          ...
          discardPlayedCard(choice2, currentPlayer, state, 1);
          discardPlayedCard(choice3, currentPlayer, state, 1);
        }
    ...
    case embargo: 
      ...
      //trash card
      discardPlayedCard(handPos, currentPlayer, state, 1);                
      return 0;
      ...
      if (choice1)
        {
          ...
          //trash card
          discardPlayedCard(choice1, currentPlayer, state, 1);        
        }
      ...
      if (index > -1)
        {
          ...
          discardPlayedCard(handPos, currentPlayer, state, 1);
          discardPlayedCard(index, currentPlayer, state, 1);
...
int playBaron(struct gameState *state, int handPos, int currentPlayer, int choice1) {
	// +1 buy
	// may discard an estate for +4 coins... if not, gain an estate for free
	// BUG: doesn't check for supply status before granting an estate; may result in negative supply
	// BUG: pre-existing, doesn't discard itself after being played
	state->numBuys++;//Increase buys by 1!
	if (choice1 > 0){ //if they choose to discard an estate card (as boolean)
		int p = 0; //Iterator for hand!
		int card_not_discarded = 1; //Flag for discard set!
		while(card_not_discarded){ // iterate over hand until estate is found
...
int playAdventurer(struct gameState *state, int handPos, int currentPlayer) {
	// dig for top 2 treasure cards
	// BUG: simulated typo, doesn't count silver cards as "treasure"
	// BUG: pre-existing bug, doesn't discard itself after being played
	// BUG: if there isn't 2 treasure, never exits the loop (had to fix this so that I could get testing coverage)
	int temphand[MAX_HAND];// moved above the if statement
	int drawntreasure=0;
	int cardDrawn;
	int z = 0;// this is the counter for the temp hand
	...
	while(drawntreasure<2) {
		...
		// if (cardDrawn == copper || cardDrawn == gold){
			...
			// then reduce the current handsize, hiding the card outside the end of the array
			// it will not be found by any other functions, and will be overwritten the next time a card is drawn
			// this effectively destroys the original and creates a copy
			// at the end, these copies are appended to the discard pile list
			// alternate idea: draw cards, keep them in your real hand, and mark the hand position of any non-treasure cards
			// then use the existing 'discard' function instead of copying to discard pile
			// cleaner and better immitation of how it really works
...
{
		// NOTE: does no safety checking! doesnt check if the handpos to discard is actually valid
		// NOTE: doesn't actually DISCARD a card, simply adds it to the played pile
		// NOTE: specifically for getting rid of action cards you play on your own turn
  ...
  if ( handPos == (state->handCount[currentPlayer] - 1) )         //last card in hand array is played
    {
      //reduce the end of the array to hide the discarded card
      state->handCount[currentPlayer]--;
    }
  else if ( state->handCount[currentPlayer] == 1 ) //only one card in hand, completely redundant
    {
    ...
    {
      //copy last card in hand to position of discarded card 
