...
int main() {
	...
	printf("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~randomtestcard1.c:   smithy~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n");
	...
	for(k=0; k<NUM_LOOPS; k++) {
		...
		G.hand[G.whoseTurn][1] = smithy;				// add the smithy card to the hand
...
sprintf(temp, "%i,",fail);strcat(failcode,temp);
		if(ret == 0) {				// drew 3 cards
			if(G.handCount[thisPlayer] != testG.handCount[thisPlayer] + 2)  {fail++;}
		} else {					// drew 0/1/2 cards, deck empty, discard empty
			if(G.deckCount[thisPlayer] != 0)  {fail++;}
			if(G.discardCount[thisPlayer] != 0)  {fail++;}
		}
sprintf(temp, "%i,",fail);strcat(failcode,temp);
...
sprintf(temp, "%i,",fail);strcat(failcode,temp);
		// properly discarded
		if(G.playedCards[G.playedCardCount-1] != smithy) {fail++;} 
