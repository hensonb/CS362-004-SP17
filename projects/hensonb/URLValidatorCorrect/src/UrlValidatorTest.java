...
public class UrlValidatorTest extends TestCase {
   ...
   public void testIsValid(Object[] testObjects, long options) {
      ...
      int printed = 0;
	  int loops = 0;		// CHANGE: added loop counter + display. can be removed. 
      if (printIndex)  {
         statusPerLine = 6;
      }
      do {
		  loops += 1;		// CHANGE: added loop counter + display. can be removed. 
         StringBuffer testBuffer = new StringBuffer();
         boolean expected = true;
         for (int testPartsIndexIndex = 0; testPartsIndexIndex < testPartsIndex.length; ++testPartsIndexIndex) {
            int index = testPartsIndex[testPartsIndexIndex]; // the list of currnt position in each list
            ResultPair[] part = (ResultPair[]) testObjects[testPartsIndexIndex]; // which list is it looking at
         ...
         // if(result == true) // CHANGE: print every URL, not just valid ones. can be removed.
	  ...
	  System.out.println("num loops = " + loops); // CHANGE: added loop counter + display. can be removed.
   }
/*
    ...
    */
	static boolean incrementTestPartsIndex(int[] testPartsIndex, Object[] testParts) {
		...
		for (int testPartsIndexIndex = testPartsIndex.length - 1; testPartsIndexIndex >= 0; --testPartsIndexIndex) {
			...
			if (carry) {
				if (index < part.length - 1) { 	// anywhere below the "max index"
					index++;
					testPartsIndex[testPartsIndexIndex] = index;
					carry = false;
					maxIndex = false;           // CHANGE: fixed the incrementer
				} else { 						// currently at the "max index"
					testPartsIndex[testPartsIndexIndex] = 0;
					carry = true;
				}
			}
			// maxIndex &= (index == (part.length - 1)); // CHANGE: removed the broken incrementer
		}
...
*/
   ...
   public static void main(String[] argv) {
	...
	System.out.println("start!"); // CHANGE: added start/end printouts, just cuz. might be removed.
      UrlValidatorTest fct = new UrlValidatorTest("url test");
      fct.setUp();
      fct.testIsValid();
      fct.testIsValidScheme();
	System.out.println("everything passed!"); // CHANGE: added start/end printouts, just cuz. might be removed.
   }
   ...
   ResultPair[] testUrlScheme = {new ResultPair("http://", true),
                               ...
                               new ResultPair("", false)    // CHANGE: the validator treats empty scheme as INVALID
	};
   ...
   ResultPair[] testUrlPort = {new ResultPair(":80", true),
                             ...
                             new ResultPair("", true),
                             new ResultPair(":65636", true), // BUG: validator treats this as valid, but it is a bad port value
   ...
   ResultPair[] testUrlQuery = {new ResultPair("?action=view", true),
                              ...
                              new ResultPair("?ilnj^&!*(&%^^[[[", true), // CHANGE: added a new query possibility, read as valid; is that correct?
                              new ResultPair("?", true), // CHANGE: added a new query possibility, read as valid; is that correct?
                              new ResultPair("", true)
   };
